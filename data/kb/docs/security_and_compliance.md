---
  doc_id: KB-011
  title: Security & Compliance Overview (SOC 2, ISO, etc.)
  owner_team: Security
...
# Security & Compliance Overview (SOC 2, ISO, etc.)
...
Enablement guidance for **Security & Compliance Overview (SOC 2, ISO, etc.)** at Northstar AI. This doc is intended to be used with related policies listed below; when policies conflict, follow the most recent **last_updated** date and escalate to the owner team.
...
- **gdpr_handling.md** (KB-012)
- **data_residency.md** (KB-013)
- **data_retention_policy.md** (KB-014)
...
- **Where do I record an exception?**  
  Record exceptions in CRM under the account and attach the approval. Include doc IDs (e.g., KB-011) and an expiry date.
...
- 2026-03-06: Content refresh, additional examples, clearer exception language.
- 2026-03-06: Effective date for the current version.

## Key fields

- **Region applicability:** US, EMEA, APAC
- **Escalation:** Security → RevOps/Legal/Security as needed
- **Citation guidance:** Cite **doc_id** + section header (e.g., `KB-011 — Exceptions & edge cases`).
